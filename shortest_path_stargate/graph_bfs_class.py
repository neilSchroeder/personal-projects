...
class graph:
    ...
    def distance_to_goal(self, n):
        """ returns euclidean distance to goal"""
        return (
            abs(self.goal_node_coords[0]-n[0])**2
            + abs(self.goal_node_coords[1]-n[1])**2
        )**0.5

    def distance_to_start(self, n):
        """ returns the euclidean distance to the start"""
        return (
            abs(self.start_node_coords[0]-n[0])**2
            + abs(self.start_node_coords[1]-n[1])**2
        )**0.5

    def topo_sort(self, keys):
        """ sorts the keys sorted first by indegree, then by dist to start """
        return sorted(
            sorted(keys,
                   key=lambda x: self.active_nodes[x].indegree),
            key=lambda x: self.distance_to_start(x))

    def search_for_goal(self):
        """ starts the directed breadth first search for the goal """
        ...
        while queue:
            ...
            if not n.is_active:
                ...
                if not n.is_start:
                    # set the indegree
                    min_indegree = min(
                        [self.nodes[key].indegree
                         for key in n.neighbors.keys()
                         if self.nodes[key].is_active])
                    n.set_indegree(min_indegree)
                # if the goal hasn't been visited continue adding to the queue
                if not self.nodes[self.goal_node_coords].is_active:
                    queue = (queue + [key for key in n.neighbors.keys()
                                      if not self.nodes[key].is_active])
                    queue_indices = {x: queue.index(x) for x in queue}
                    queue.sort(key=lambda x:
                               0.35*queue_indices[x]
                               + 3*n.indegree
                               + 0.32*self.distance_to_goal(x))
                n.activate()

            # allows user to watch the search progression
    ...
    def get_path_length(self, path):
        """ gets the total euclidean length of a given path """
        return sum(
            [self.nodes[path[i]].distance_to_node(path[i+1])
             for i in range(len(path)-1)])

    def find_shortest_path(self):
        """
        check activated nodes and determine the shortest path to the start
        """
        queue = [self.start_node_coords]
        parent_path_length = {}
        ...
        while queue:
            ...
            if not queue.count(self.goal_node_coords):
                neighbors = [x for x in self.nodes[key].neighbors
                             if self.nodes[x].is_active
                             and
                             self.nodes[x].indegree > self.nodes[key].indegree
                             and queue.count(x) < 2]
                queue = queue + neighbors
                for neighbor in neighbors:
                    if neighbor in parent_path_length.keys():
                        parent_length = (
                            self.nodes[key].distance_from_start_node
                        )
                        if parent_length < parent_path_length[neighbor]:
                            parent_path_length[neighbor] = parent_length
                    else:
                        parent_path_length[neighbor] = (
                            self.nodes[key].distance_from_start_node
                        )
            ...
            queue = sorted(
                sorted(queue,
                       key=lambda x: self.active_nodes[x].indegree),
                key=lambda x: parent_path_length[x])
            if self.watch_evolution:
                self.draw_path(queue, reset=True, symb='Q')
                time.sleep(0.005)
        ...
        self.active_nodes[gnc].set_distance_from_start(self.active_nodes)
