class node:
    ...
    def set_distance_from_start(self, active_nodes):
        ...
        for key in self.neighbors.keys():
            ...
            if active_nodes[key].is_start:
                ...
                return
            if self.distance_to_node(key) + active_nodes[key].distance_from_start_node <= self.distance_from_start_node:
                self.distance_from_start_node = self.distance_to_node(key) + active_nodes[key].distance_from_start_node
                min_key = key
    ...
    def get_neighboring_nodes(self):
        ...
        possible_neighbors = [(i,j) for i in range(i_min, i_max) for j in range(j_min, j_max)]
